...
const UpdateGroupChatModal = ({fetchAgain,setFetchAgain,fetchMessages}) => {
    ...
    const handleAddUser=async(userToAdd)=>{
        if(Selecthat.users.find((user)=>user._id===userToAdd._id))
        {
            toast({
    ...
    const handleRemove=async(userToRemove)=>{
        if(Selecthat.groupAdmin._id!==user._id&&userToRemove._id!==user.Id)
        {
            toast({
        ...
        try {
            ...
            userToRemove._id===user._id?setSelecthat():setSelecthat(data)
            setLoading(false)
            fetchMessages()
            setFetchAgain(!fetchAgain)
  ...
  return (
      <>
        ...
        <ModalContent>
            ...
            >{Selecthat.chatName}</ModalHeader>
            <ModalCloseButton />
            <ModalBody>
                <Box  d='flex' flexWrap='wrap' w={"100%"} pb={3}>
                    {Selecthat.users.map(user=>(
